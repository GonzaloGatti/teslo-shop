...
export const ProductForm = ({ product, categories }: Props) => {
  ...
  const onSizeChange = (selectedSize: string) => {

    const sizes = new Set(getValues('sizes'))
    sizes.has(selectedSize) ? sizes.delete(selectedSize) : sizes.add(selectedSize)
  ...
  const onSubmit = async(data: FormInputs) => {
    ...
    formData.append('price', restData.price.toString())
    formData.append('inStock', restData.inStock.toString())
    formData.append('sizes', restData.sizes.toString())
    formData.append('tags', restData.tags)
    formData.append('categoryId', restData.categoryId)
    formData.append('gender', restData.gender)
            ...
            
            {
              sizes.map( size => (
                <div 
                    ...
                    className={
                        clsx(
                            'flex items-center justify-center w-14 h-10 mr-2 border rounded-md transition-all cursor-pointer',
                            {
